...
def validate_coupon(db, code, booking_amount):
    ...
    if booking_amount < float(coupon.min_booking_amount): return None, f"Minimum Rs.{coupon.min_booking_amount} required"
    ...
    return {"coupon_id": coupon.id, "code": coupon.code, "discount_amount": round(discount, 2)}, None
