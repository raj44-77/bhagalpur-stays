...
def get_hotels(db, city_slug=None, area_slug=None, search=None, status="approved", skip=0, limit=20):
    ...
    result = []
    for h in hotels:
        min_price = db.query(func.min(RoomType.base_price)).filter(RoomType.hotel_id == h.id, RoomType.is_active == True).scalar()
        ...
        result.append({"id": h.id, "name": h.name, "slug": h.slug, "star_rating": h.star_rating, "status": h.status, "is_featured": h.is_featured, "city_name": h.city.name if h.city else None, "primary_image": primary_image, "avg_rating": round(float(avg_rating), 1) if avg_rating else None, "review_count": review_count or 0, "min_price": float(min_price) if min_price else None})
