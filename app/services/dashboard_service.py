...
def get_owner_dashboard(db, owner_id):
    ...
    avg_rating = db.query(func.avg(Review.rating)).filter(Review.hotel_id.in_(hotel_ids)).scalar() or 0
    recent = db.query(Booking).filter(Booking.hotel_id.in_(hotel_ids)).order_by(Booking.created_at.desc()).limit(20).all()
    return {"kpis": {"total_bookings": total_bookings, "monthly_revenue": float(revenue), "avg_rating": round(float(avg_rating), 1), "total_hotels": len(hotel_ids)}, "recent_bookings": [{"id": b.id, "booking_ref": b.booking_ref, "guest_name": b.guest_name or "N/A", "room_type": b.room_type.name if b.room_type else "N/A", "check_in": str(b.check_in), "total_amount": float(b.total_amount), "status": b.status, "payment_status": b.payment_status} for b in recent]}
